...
class MyCircularPercentIndicator extends StatefulWidget {
  ...
  final Widget? widgetIndicatorLeftSide;
  final Widget? widgetIndicatorRightSide;
  ...
  MyCircularPercentIndicator({
    ...
    this.onAnimationEnd,
    this.widgetIndicatorLeftSide,
    this.widgetIndicatorRightSide,
  ...
  @override
  _MyCircularPercentIndicatorState createState() =>
      _MyCircularPercentIndicatorState();
}

class _MyCircularPercentIndicatorState extends State<MyCircularPercentIndicator>
  ...
  void _checkIfNeedCancelAnimation(MyCircularPercentIndicator oldWidget) {
  ...
  @override
  void didUpdateWidget(MyCircularPercentIndicator oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    double fixedStartAngle = widget.startAngle;
    double startAngleFixedMargin = 1.0;
    if (widget.arcType != null) {
      final arcAngles = _getStartAngleFixedMargin(widget.arcType!);
      fixedStartAngle = arcAngles.fixedStartAngle;
      startAngleFixedMargin = arcAngles.startAngleFixedMargin;
    }
    final start = radians(-90.0 + fixedStartAngle-((widget.percent*360)/2*startAngleFixedMargin)).toDouble();
    final startX = (widget.radius-widget.lineWidth/2) * math.cos(start);
    final startY = (widget.radius-widget.lineWidth/2) * math.sin(start);
    final endX = -(widget.radius-widget.lineWidth/2)* math.cos(-start);
    final endY = -(widget.radius-widget.lineWidth/2) * math.sin(-start);
      ...
      Container(
        ...
        child: Stack(
          children: [
            ...
            if (widget.widgetIndicatorRightSide != null && widget.animation)
              Positioned.fromRect(
                rect: Rect.fromCircle(center: center, radius: 10),
                child: Transform.rotate(
                  angle: radians(
                      ...
                      .toDouble(),
                  child: Transform.rotate(
                    angle: radians(0).toDouble(),//getCurrentPercent(_percent),
                    child: Transform.translate(
                      ...
                      child: widget.widgetIndicatorRightSide,
                    ),
                  ),
                ),
              ),
            if (widget.widgetIndicatorLeftSide != null && widget.animation)
              Positioned.fromRect(
                rect: Rect.fromCircle(center: center, radius: 10),
                child: Transform.rotate(
                  angle: radians(
                  ...
                  child: Transform.rotate(
                    ...
                    child: Transform.translate(
                      ...
                      child: widget.widgetIndicatorLeftSide,
                    ),
                  ),
                ),
              ),
...
class _CirclePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (reverse) {
      ...
      final start = radians(-90.0 + fixedStartAngle-(progress/2*startAngleFixedMargin)).toDouble();
      final end = radians(progress * startAngleFixedMargin).toDouble();
      canvas.drawArc(
        Rect.fromCircle(
          center: center,
